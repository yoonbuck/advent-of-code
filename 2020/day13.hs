...
solveA (time, busses) = uncurry (*)
  ...
  $ map (\bus -> (bus - (time `mod` bus), bus)) busses

solveB :: String -> Integer
solveB = head
  . foldr (\p (a:b:_) -> filter p [a, b ..]) [1 ..]
  . map ((\(a, b) n -> n `mod` b == ((-a) `mod` b)) . second read)
  . filter ((/= "x") . snd)
  . zip [0 ..]
  . splitOn ","
  . head
...
{-
  ...
  This is what we want, but the correct answer (432787540701585) is large and
